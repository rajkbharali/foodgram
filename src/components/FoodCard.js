    ...
    >
      ...
      >
        {reqObj?.info?.aggregatedDiscountInfoV3 ? (
          <p className="m-3">
            {reqObj?.info?.aggregatedDiscountInfoV3?.header +
              " " +
              reqObj?.info?.aggregatedDiscountInfoV3?.subHeader}
          </p>
        ) : (
          ""
        )}
      ...
      <div className="mx-2">
        <p className="text-lg whitespace-nowrap overflow-hidden">
          <b>{reqObj?.info?.name}</b>
        </p>
        <p className="flex items-center">
          <MdStars className="mr-1 text-green-700" />{" "}
          <b className="mr-1">{reqObj?.info?.avgRatingString}</b> |{" "}
          <b className="ml-1">{reqObj?.info?.sla?.slaString}</b>
        </p>
        <p className="text-sm whitespace-nowrap overflow-hidden text-gray-500">
          {reqObj?.info?.cuisines.join(", ")}
        </p>
        <p className="text-sm whitespace-nowrap overflow-hidden text-gray-500">
          {reqObj?.info?.locality}
        </p>
      </div>
    </div>
  );
...
export const highRatedRestaurants = (FoodCard) => {
  return (props) => {
    return (
      <>
        <label className="absolute z-10 m-2 px-3 py-1 bg-black text-white text-sm rounded-2xl">
          TOP RATED
        </label>
        <FoodCard {...props} />
      </>
